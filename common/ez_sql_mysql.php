	...
	*/
	...
	class ezSQL_mysql extends ezSQLcore
	{
		...
		*/
		...
		function quick_connect($dbuser='', $dbpassword='', $dbname='', $dbhost='localhost', $encoding='utf8')
		{
			$return_val = false;
		...
		*/
        ...
        public static function get_db($db_name)
        {
            $db_config = get_db_config($db_name);
            if (!$db_config)
            {
                trigger_error("DB(".$db_name.") config isnot existed.",E_USER_WARNING);
                return false;
            }
            $db = new self($db_config['DB_USER'], $db_config['DB_PWD'], $db_config['DB_NAME'], $db_config['DB_HOST']);
            return $db;
        }
	}
